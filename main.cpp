...
bool LoadMedia()
{
	...
	gMusic = Mix_LoadMUS("amthanh/sound_bkgr_audio.wav");
	if (gMusic == nullptr)
	{
	...
	gClick = Mix_LoadWAV("amthanh/sound_mouse_click.wav");
	if (gClick == nullptr)
	{
	...
	gLose = Mix_LoadWAV("amthanh/sound_lose_sound.wav");
	if (gLose == nullptr)
	{
	...
	{
		gFont = TTF_OpenFont("phongchu/Roboto-Regular.ttf", 28);
		if (gFont == NULL)
		{
		...
		else
		{
			...
			if (!gMenuTexture.LoadFromFile("Anh/menu2.png", gRenderer))
			{
				cout << "Failed to load menu image" << endl;
				success = false;
			}

			if (!gInstructionTexture.LoadFromFile("Anh/instruction.png", gRenderer))
			{
				cout << "Failed to load instruction image" << endl;
				success = false;
			}

			if (!gPlayButtonTexture.LoadFromFile("Anh/play_button.png", gRenderer))
			{
			...
			if (!gHelpButtonTexture.LoadFromFile("Anh/help_button.png", gRenderer))
			{
			...
			if (!gBackButtonTexture.LoadFromFile("Anh/back_button.png", gRenderer))
			{
			...
			if (!gExitButtonTexture.LoadFromFile("Anh/exit_button.png", gRenderer))
			{
			...
			if (!gPauseButtonTexture.LoadFromFile("Anh/pause_button.png", gRenderer))
			{
			...
			if (!gContinueButtonTexture.LoadFromFile("Anh/continue_button.png", gRenderer))
			{
			...
			if (!gGroundTexture.LoadFromFile("Anh/ground.png", gRenderer))
			{
				cout << "Failed to load ground image" << endl;
				success = false;
			}

			if (!gCharacterTexture.LoadFromFile("Anh/char.png", gRenderer))
			{
			...
			{
				gCharacterClips[0].x = 60 * 0;
				gCharacterClips[0].y = 0;
				gCharacterClips[0].w = 60;
				gCharacterClips[0].h = 60;

				gCharacterClips[1].x = 60 * 1;
				gCharacterClips[1].y = 0;
				gCharacterClips[1].w = 60;
				gCharacterClips[1].h = 60;

				gCharacterClips[2].x = 60 * 2;
				gCharacterClips[2].y = 0;
				gCharacterClips[2].w = 60;
				gCharacterClips[2].h = 60;

				gCharacterClips[3].x = 60 * 3;
				gCharacterClips[3].y = 0;
				gCharacterClips[3].w = 60;
				gCharacterClips[3].h = 60;

				gCharacterClips[4].x = 60 * 4;
				gCharacterClips[4].y = 0;
				gCharacterClips[4].w = 60;
				gCharacterClips[4].h = 60;

				gCharacterClips[5].x = 60 * 5;
				gCharacterClips[5].y = 0;
				gCharacterClips[5].w = 60;
				gCharacterClips[5].h = 60;
			}

			if (!gLoseTexture.LoadFromFile("Anh/lose.png", gRenderer))
			{
